...
>
> This document specifies the naming conventions for mathlib. Key guidelines include:
>
> *   **File Names:** Use `UpperCamelCase` for all `.lean` files (e.g., `FileName.lean`).
> *   **Capitalization:**
>     *   `UpperCamelCase` for types and `Prop`s (e.g., `MonoidHom`, `IsField`).
>     *   `lowerCamelCase` for terms of `Type`s (e.g., `toFun`, `hPow`).
>     *   `snake_case` for theorems and proofs (terms of `Prop`) (e.g., `map_one`, `gt_iff_lt`).
>     *   Functions follow the convention of their return type.
> *   **Spelling:** All declaration names must use American English (e.g., `factorization`, not `factorisation`).
> *   **Theorem Naming:**
>     *   Names should be descriptive, often stating the conclusion (e.g., `mul_zero`).
>     *   Use `_of_` to introduce hypotheses (e.g., `lt_of_le_of_ne`).
>     *   Follow a standard dictionary for symbols (e.g., `∧` becomes `and`, `∈` becomes `mem`).
> *   **Structural Lemmas:** Use standard suffixes for common properties, such as `.ext` for extensionality, `_inj` for injectivity, and `_mono` for monotonicity.
> *   **Namespaces:** Use dot notation for namespaced theorems (e.g., `And.intro`, `Eq.symm`).
