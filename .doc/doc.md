> **Summary of Documentation Style Requirements**
>
> This document outlines the documentation standards for mathlib. Key points for contributors:
>
> *   **File Header:** Every file must start with a copyright header, author list, imports, and a module docstring (`/-! ... -/`).
> *   **Module Docstring:** The module docstring must include a title (`#`), a summary, and optional sections for main definitions, main statements, notation, implementation notes, references, and tags.
> *   **Declaration Docstrings:** All public definitions and major theorems require a docstring (`/-- ... -/`) describing their mathematical purpose.
> *   **Formatting:** Use Markdown for general text and LaTeX for mathematical expressions (`$...$` for inline, `$$...$$` for display).
> *   **Code References:** Enclose Lean identifiers in backticks (e.g., `` `finset.card_pos` ``) to create links in the generated documentation.
> *   **Citations:** Reference works from the central `.bib` file using `[citationKey]` or `[Link text][citationKey]`.
