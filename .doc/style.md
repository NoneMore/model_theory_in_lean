...
>
> This document provides style guidelines for writing Lean code in mathlib.
>
> *   **Line Length:** Keep lines under 100 characters.
> *   **File Structure:**
>     *   Start with a copyright header and author list.
>     *   Follow with imports, each on a new line.
>     *   Add a module docstring (`/-! ... -/`) summarizing the file's content.
> *   **Formatting Declarations:**
>     *   Do not indent top-level commands (`def`, `theorem`, `namespace`, `section`).
>     *   Indent proofs and definition bodies by 2 spaces.
>     *   For multi-line statements, indent subsequent lines by 4 spaces.
> *   **Tactic Mode:**
>     *   Place `by` at the end of the line preceding the tactic block (e.g., `:= by`).
>     *   Indent tactics within the block. Use a dot (`·`) to focus on subgoals.
> *   **Calculations:** Use `calc` for calculational proofs, indenting the block and aligning relations.
> *   **Whitespace:** Use spaces around operators (`:`, `:=`, `→`). Prefer `<|` over `$`.
> *   **Comments:** Use `/-- -/` for declaration docstrings, `/-! -/` for section headers, and `--` for inline comments.
> *   **Clarity:** Prefer placing hypotheses to the left of the colon. Use `fun x ↦ ...` for anonymous functions.
